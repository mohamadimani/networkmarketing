...
<div class="content-page">
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <h4 class="page-title">مدیریت دسته بندی نمونه کارها</h4>
            ...
            <div class="row">
                <div class="col-sm-12 col-md-8">
                    <div class="card-box">
                        ...
                        <?php }
                        if (!empty($_SESSION["add_demo_category"]) and $_SESSION["add_demo_category"] == "success") {
                            ?>
                            <div class="alert alert-success alert-dismissable text-center">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                        ...
                        <?php }
                        if (!empty($_SESSION["add_demo_category"]) and $_SESSION["add_demo_category"] == "danger") {
                            ?>
                        ...
                        <?php }
                        if (!empty($_SESSION["add_demo_category"]) and $_SESSION["add_demo_category"] == "warning") {
                            ?>
                        ...
                        <?php }
                        if (!empty($_SESSION["add_demo_category"]) and $_SESSION["add_demo_category"] == "empty") {
                            ?>
                            <div class="alert alert-warning alert-dismissable text-center">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                        ...
                        unset($_SESSION["add_demo_category"]);
                        if (empty($category_id)) {
                            ?>
                            <h4 style="margin: 10px 0 30px 0;"><b> افزودن دسته اصلی جدید </b></h4>
                        <?php } else if (!empty($category_id)) { ?>
                            <h4 style="margin: 10px 0 30px 0;"><b> افزودن زیر دسته به (
                                    <span style="color: #ff3c15"><?= $category_info["title"] ?></span>)</b>
                                <a href="<?= SITE_URL ?>admin_demo/category/">
                        ...
                        <form action="<?= SITE_URL ?>admin_demo/add_category/<?= $cat_id ?>"
                        ...
                        <table class="table table-hover mails m-0 table table-actions-bar m-t-20">
                            ...
                            foreach ($categorys as $item) {
                                ...
                                <tr class="active">
                                    ...
                                    <?php if (empty($category_id)) { ?>
                                        <td>
                                            <a href="<?= SITE_URL ?>admin_demo/category/<?= $item["id"] ?>">
                                    ...
                                    <td>
                                        <!-- ==========Edit ====== -->
                                        <a href="<?= SITE_URL ?>admin_demo/edit_category/<?= $item['id'] ?>"
                                           title="ویرایش"
                                           class="table-action-btn ">
    ...
    <script>
        ...
        function active_user(id, status, item) {
            var url = "<?= SITE_URL ?>admin_demo/category_status_change";
        ...
        function delete_category(id, item) {
            var url = "<?= SITE_URL ?>admin_demo/delete_category";
            var data = {'id': id};
            $.post(url, data, function (msg) {
                if (msg == true) {
