    ...
    <div class="col-sm-8 col-sm-push-3 col-xs-8 col-xs-push-3 ">
        <div class="card-box">
            ...
            if (!empty($_SESSION["update_clerk"]) and $_SESSION["update_clerk"] == "success") {
                ...
                <div class="alert alert-success alert-dismissable text-center">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                    <a class="alert-link"> اطلاعات کاربر با موفقیت بروزرسانی شد</a>
                </div>
            <?php }
                            ...
                            <div class="col-sm-4">
                                <a href="<?= SITE_URL ?>admin_add_user" data-plugin="custommodal"
                                   ...
                                   data-overlaySpeed="200" data-overlayColor="#36404a">
                                    <i class="md md-add"></i>افزودن کاربر جدید</a>
                            </div>
                        </div>
                    </tr>
                    <tr>
                        ...
                        <tr class="active">
                            ...
                            <td>
                                <a style="cursor: pointer" onclick="active_user(<?= $users['id'] ?>,'inactive',this)"
                                   title="غیر فعال کردن"
                                   class="table-action-btn <?php if ($users['status'] == 'INACTIVE' or $users['username'] == "admin") {
                                       echo 'hidden';
                                   } ?>">
                            ...
                            <td>
                                <a href="<?= SITE_URL ?>admin_clerk_list/clerks_access/<?= $users['id'] ?>"
                                   title="دسترسی ها" class="table-action-btn <?php if ($users['username'] == "admin") {
                                    echo 'hidden';
                                } ?> ">
                                    <i class="btn btn-danger   btn-xs fa  fa-key"
                                       style="padding: 4px 5px !important;"></i></a>
                            </td>
                            <td>
                                <a href="<?= SITE_URL ?>admin_add_user/edit_clerk/<?= $users['id'] ?>"
                                   title="  ویرایش" class="table-action-btn  <?php if ($users['username'] == "admin") {
                                    echo 'hidden';
                                } ?>">
...
<script>
    ...
    function active_user(id, status, item) {
        var url = "<?= SITE_URL ?>admin_clerk_list/user_status_change";
