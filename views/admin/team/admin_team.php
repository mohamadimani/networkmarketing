            ...
            <div class="row port m-t-20 m-b-20">
                <div class="portfolioContainer ">
                    <div class="col-sm-12 col-xs-12  col-md-6 webdesign illustrator">
                        <form action="<?= SITE_URL ?>admin_team/upload_gallery" method="post"
                    ...
                    foreach ($gallery as $row) { ?>
                        <div class="col-sm-6 col-lg-3 m-t-20 col-md-4 webdesign illustrator">
                            <div class="gal-detail box ">
                                ...
                                <img src="<?= SITE_URL ?>public/images/team/<?= $row['img_name'] ?>"
                                     class="thumb-img" alt="<?= $row['user_name'] ?>" style="width: 100%;height: 150px;">
                                <!--                                <h4>  --><? //= $row['title'] ?><!-- </h4>-->
                                <!--                                <div>-->
                                <!--                                    <label class="col-sm-8 control-label" for="m1">دسته بندی</label>-->
                                <!--                                    <div class="">-->
                                <!--                                        <select onchange="set_category(this,<? //= $row["id"] ?>)"
//                                                name="category" id="m1" required class="input-sm form-control">
...
//                                            <?php
                                ...
                                <div>
                                    <label class="col-sm-6 control-label" for="m1">نام</label>
                                    <div class="">
                                        <input id="m1" onblur="set_link(this,<?= $row['id'] ?>)" type="text"
                                               class="input-sm form-control" name="user_name" value="<?= $row['user_name'] ?>">
                                    </div>
                                </div>

                                <div>
                                    <label class="col-sm-6 control-label" for="m2">سمت</label>
                                    <div class="">
                                        <input id="m2" onblur="set_alt(this,<?= $row['id'] ?>)" type="text"
                                               class="input-sm form-control" name="occupation" value="<?= $row['occupation'] ?>">
                                    </div>
                                </div>
...
//                                        <?php //if ($row['slider'] == "YES") {
    ...
    <script>
        ...
        function set_category(item, img_id) {
            var cat_id = $(item).val();
            var url = "<?= SITE_URL ?>admin_team/update_category";
        ...
        function set_check(item, id) {
            ...
            var url = "<?= SITE_URL ?>admin_team/status";
            if ($(item).prop('checked')) {
                var data = {"name": item_name, "id": id, "status": "active"};
                $.post(url, data, function (msg) {
                    if (msg == true) {
                        swal("ثبت شد", " ", "success")
                    } else {
        ...
        function set_link(item, id) {
            ...
            var url = "<?= SITE_URL ?>admin_team/user_name";
        ...
        function set_alt(item, id) {
            var item_value = $(item).val();
            var url = "<?= SITE_URL ?>admin_team/occupation";
            var data = {"value": item_value, "id": id};
            $.post(url, data, function (msg) {
                if (msg == true) {
                    swal(" ذخیره شد ", " ", "success");
        ...
        function delete_img(id, item) {
            var url = "<?= SITE_URL ?>admin_team/delete_img";
            var data = {"id": id};
            $.post(url, data, function (msg) {
                if (msg == true) {
